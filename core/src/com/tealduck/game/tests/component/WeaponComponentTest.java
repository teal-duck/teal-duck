...
public class WeaponComponentTest {
	...
	public void testFireWeapon() {
		...
		weaponComponent.fireWeapon(entityEngine, shooter, position, direction, team); // breaks here. Too tired to figure out. Ask Ben in morning.
	...
	public void testStopReloading(){
		...
		Assert.assertEquals(weaponComponent.reloadTime, 0.0f, DELTA);
		Assert.assertEquals(weaponComponent.bulletReloadTime, 0.0f, DELTA);
	...
	public void testStartReloading(){
		...
		Assert.assertEquals(weaponComponent.bulletReloadTime, 0f, DELTA);
		weaponComponent.extraAmmo = 0;
		weaponComponent.bulletReloadTime = 5f;
		weaponComponent.reloadTime = 3f;
		weaponComponent.ammoInClip = 8;
		weaponComponent.startReloading();
		Assert.assertEquals(weaponComponent.reloadTime, 3f, DELTA);
		Assert.assertEquals(weaponComponent.bulletReloadTime, 5f, DELTA);
	...
	public void testDoCooldown(){
		...
		Assert.assertEquals(weaponComponent.cooldownTime, 5f, DELTA);
		weaponComponent.doCooldown(5f);
		Assert.assertEquals(weaponComponent.cooldownTime, 0f, DELTA);
		weaponComponent.doCooldown(5f);
		Assert.assertEquals(weaponComponent.cooldownTime, 0f, DELTA);
