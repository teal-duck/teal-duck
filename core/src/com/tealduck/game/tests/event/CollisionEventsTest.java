...
public class CollisionEventsTest {
	...
	public void testHandleKnockback() {
		...
		//test same team
		Assert.assertEquals(entityManager.getComponent(e2, MovementComponent.class).velocity, new Vector2(0.0f,22.5f));
		Assert.assertEquals(entityManager.getComponent(e1, MovementComponent.class).velocity, new Vector2(0.0f,0.0f));
		...
		Assert.assertEquals(((entityManager.getComponent(e3, MovementComponent.class).acceleration)),new Vector2(0,60000));
		//System.out.println(entityManager.getComponent(e3,  ChaseComponent.class).searchDirection);
		Assert.assertEquals((entityManager.getComponent(e3,  ChaseComponent.class).searchDirection), new Vector2(0,1)); //Test gets chase component
	...
	public void testHandleDamage() {
		...
		Assert.assertEquals(entityManager.getComponent(e2, HealthComponent.class).health, 2); //testing took damage
		
		CollisionEvents.handleDamage(entityEngine, e3, e2);
		
		Assert.assertEquals(entityManager.getComponent(e2, HealthComponent.class).health, 2); // testing no further damage from ally
		
		CollisionEvents.handleDamage(entityEngine, e1, e2);
		
		Assert.assertEquals(entityManager.getComponent(e2, HealthComponent.class).health, 0); // Testing minimum health of 0 with no score component
		...
		Assert.assertEquals(entityManager.getComponent(e1, ScoreComponent.class).workingScore, 100 ); // assert that handleScoreForEntityDeath was called
	...
	public void testHandleScoreForEntityDeath() {
		...
		Assert.assertEquals(entityManager.getComponent(shooterEntity, ScoreComponent.class).workingScore, 100); // assert that the score component of the 
																				//shooter entity that fired the bullet is awarded the score for the kill
	}
		@Test
		public void testHandlePickup() {
			...
			Assert.assertEquals(entityManager.getComponent(receiverEntity, WeaponComponent.class).extraAmmo, 15); // assert pickup function works
			
			Assert.assertTrue(entityEngine.isEntityFlaggedToRemove(senderEntity)); // assert pickup flagged for removal
